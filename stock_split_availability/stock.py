...
class stock_move(orm.Model):
    ...
    def _get_context_check(self, cr, uid,
        move, what=('in'), states=('assigned','done'), from_date=False,
        to_date=False, context=None):
        return {
            'shop': False,
            'warehouse': False,
    ...
    def _get_specific_available_qty(self, cr, uid, move, context=None):
        ...
        if move:
            ...
            if not states_out:
                states_out = states
            from_date_in = context.get('from_date_in')
            to_date_in = context.get('to_date_in')
            from_date_out = context.get('from_date_out')
            to_date_out = context.get('to_date_out')
            if from_date_in is None:
                from_date_in = context.get('from_date', False)
            if to_date_in is None:
                to_date_in = context.get('to_date', False)
            if from_date_out is None:
                from_date_out = context.get('from_date', False)
            if to_date_out is None:
                to_date_out = context.get('to_date', False)
            c.update(self._get_context_check(
                cr, uid, move, what=what, states=states_in,
                from_date=from_date_in, to_date=to_date_in, context=context))
            stock = product_obj.get_product_available(
                cr, uid, [move.product_id.id], context=c)
            incoming_qty = stock.get(move.product_id.id, 0.0)
            what = ('out',)
            c.update(self._get_context_check(
                cr, uid, move, what=what, states=states_out,
                from_date=from_date_out, to_date=to_date_out, context=context))
            stock = product_obj.get_product_available(
                cr, uid, [move.product_id.id], context=c)
...
#            else:
            ...
            if quantity_rest:
                ...
                procurement = move.sale_line_id.procurement_id
                procurement_obj.write(cr, uid, procurement.id, {'move_id': new_move_id}, context=context)
        return new_move_id
