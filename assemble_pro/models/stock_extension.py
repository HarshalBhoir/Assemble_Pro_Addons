...
class stock_production_lot_extension(models.Model):
    ...
    def onchange_internal_reference(self):
        if self.name and self.product_id:
            products = a = self.env['product.product'].search([('id', '=', self.product_id.id)])
            for product_barcode in products:
                for barcodes in product_barcode.barcode_ids:
                    if self.name == barcodes.barcode_scanned:
                        self.ref = barcodes.barcode_dot
...
class stock_pack_operation_lot_extension(models.Model):
    ...
    picking_type_code = fields.Selection([('incoming', 'Suppliers'), ('outgoing', 'Customers'), ('internal', 'Internal'),('dc','=','DC')], 'Type of Operation' ,related="operation_id.picking_type_code" , store=True )
    ...
    def onchange_length(self):
        if self.start_length and  self.end_length:
            ...
            if (self.qty != self.operation_id.product_qty) and (self.picking_type_code == 'outgoing'):
                raise ValidationError("Inputed quantity is incorrect \n Correct Quantity is %s" % (self.operation_id.product_qty))
            else:
            ...
            # if self.start_length ==0 and  self.end_length == 0:
            #     raise ValidationError("Kindly enter Start and End Length of product" )
...
class stock_barcode_list(models.Model):
    ...
    pack_operation_id = fields.Integer(string='Stock Pack Operation', track_visibility='always')
...
class stock_picking_extension(models.Model):
    ...
    modvat_boolean = fields.Boolean(String="Modvat")
        ...
        if self.import_text==True:
            ...
            for record in self.move_lines:
                if len(quant):
                    quant_found = False
                    for quant_id in quant:
                        # print "1111111111111111111111111111" ,  quant_id.name
                        if record.product_id.id == quant_id.product_id.id:
                            quant_found = True
                            if record.product_id.name in quants_dict:
                                quants_dict[record.product_id.name] = quants_dict[record.product_id.name] + quant_id.qty
...
"
            for quant_name, value in quants_dict.iteritems():
                # print "KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK" , quant_name, value
                if value <= 0:
                    raise_error = True
                    quant_string += "\n%s -- -- -- ( %s ) " % (quant_name,value)
            if raise_error:
    ...
    def write(self ,vals):
        ...
        if self.barcode_product:
            ...
            for barcode in my_list:
                # print "BBBBBBBBBBBBBBB barcode_list_ids" , barcode
                ...
                if self.picking_type_code == 'outgoing':
                    ...
                    barcode_list = self.barcode_list_ids.search([])
                    for a in barcode_list:
                        # print "KKKKKKKKKKKKKKKKKKKKKKKKK" , a , a.name
                        if a.name != barcode:
                            self.barcode_list_ids.create({'name':barcode,'picking_id':self.id})
                    if not barcode_list:
                        self.barcode_list_ids.create({'name':barcode,'picking_id':self.id})
    ...
    def get_po_to_split_from_barcode(self, barcode):
        print "AAAAAAAAAAAAAA get_po_to_split_from_barcode stock_picking"
    ...
    def _check_product(self, product, og_barcode, qty=1.0):
        ...
        # print "AAAAAAAAAAAAAAAAAAAAAAAAA _check_product"
        barcode_product = []
        ...
        for ex_barcode in existing_barcodes:
            if og_barcode == ex_barcode.name:
                raise UserError("You have already scanned this serial number which exists in Stock Barcode List")
    ...
    def on_barcode_scanned(self, barcode):
        # print "AAAAAAAAAAAAAAAAAAAAAAAAA stock_picking AAAA on_barcode_scanned"
        og_barcode = barcode
        line_list = barcode.split('-')
        line_tot = ''.join(line_list[0].strip())
        barcode =  line_tot
        ...
        for rec in self.search([('picking_type_code', '=', 'incoming'),('state', '=', 'done')]) :
            ...
            for record in rec.pack_operation_product_ids:
                if record.modvat_applicable:
                    modvat_list.append(record)
            if modvat_list and rec.gate_pass == 'no':
                rec.modvat_boolean = True
            else:
                rec.modvat_boolean = False
    
    @api.multi
    def get_line_value(self,rec):
        metric_uom = self.env['ir.model.data'].xmlid_to_object('product.product_uom_meter')
        lot_reference = "; ".join([x.lot_reference for x in rec.pack_lot_ids if x.lot_reference])
        # print "LLLLLLLLLLLLLLLL",lot_reference
        if rec.product_uom_id.id == metric_uom.id:
            if rec.pack_lot_ids:
                ...
                rec_line = [
                    [rec.product_id.default_code,rec.product_id.name,lot_reference,0,rec.qty_done,rec.product_uom_id.name,""],
            ...
            rec_line = [rec.product_id.default_code,rec.product_id.name,lot_reference,0,rec.qty_done,rec.product_uom_id.name,""]
        return rec_line
...
class stock_pack_operation_extension(models.Model):
    ...
    # barcode_list_ids = fields.Char(string='Barcode List' )#, compute="add_barcode" ## 'stock.barcode.list', 'pack_operation_id', 
    ...
    def write(self ,vals):
        ...
        barcode_list_obj = self.env['stock.barcode.list']
        if 'barcode_list' in vals and vals.get('barcode_list'):
            my_list = str(vals.get('barcode_list')).split(",")
            for barcode in my_list:
                if self.picking_id and self.picking_type_code == 'outgoing':
                    barcode_list = barcode_list_obj.search([('pack_operation_id','=',self.id)])
                    barcode_list_names = [a.name for a in barcode_list]
                    if barcode not in barcode_list_names:
                        barcode_list_obj.create({'name':barcode,'pack_operation_id':self.id,'picking_id':self.picking_id.id})
        
        if 'pack_lot_ids' in vals and vals.get('pack_lot_ids'):
            for a in self.pack_lot_ids:
                if (a.qty != self.product_qty) \
                        and (self.product_uom_id.category_id.name == 'm') \
                        and  (self.picking_type_code == 'outgoing'):
                    raise ValidationError("Inputed quantity is incorrect \n Correct Quantity is %s" % (self.product_qty))
                if self.product_uom_id.category_id.name == 'm' and self.picking_id.picking_type_code == 'outgoing':
                    if a.start_length == 0 and  a.end_length == 0:
                        raise ValidationError("Kindly enter Start and End Length of product" )
    ...
    def on_barcode_scanned(self, barcode):
        ...
        # print "LLLLLLLLLLLLLLLL len(barcode)" , len(barcode) , self.batch_length , self.picking_id.picking_type_code
        
        if len(barcode) != int(self.batch_length) and self.picking_type_code == 'incoming':
            raise ValidationError("You have scanned a serial number whose batch length does not match with this product " )
        ...
        if barcode in barcode_list:
            return { 'warning': {
                'title': _('You have entered this serial number already'),
                'message': _('You have already scanned a serial number which exists in Stock Barcode List'),
            }}
        else:
        ...
        if context.get('only_create') and context.get('serial'):
            if barcode in [x.lot_name for x in self.pack_lot_ids]:
                ...
                # print "OOOOOOOOOOOOOOOOOOOOOOOOOOOOO self.pack_lot_ids"
            ...
            if corresponding_pl:
                ...
                if lots:
                    ...
                    if og_barcode in code:
                        ...
                        self.pack_lot_ids += self.pack_lot_ids.new({'qty': 1.0, 'lot_id': product_serial_no2})
                        raise_exception_serial = False
                        # print "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"

                        
# -------------------------  PRevious Code for Packlist ###########
...
# -------------------------  PRevious Code for Packlist ###########
                    ...
                    if  raise_exception_serial:
                        ...
                        return { 'warning': {
                                    'title': _('You have entered a wrong Serial Number of product'),
                                    'message': _('You have scanned the serial number which does not belong to this product '),
                                }}
