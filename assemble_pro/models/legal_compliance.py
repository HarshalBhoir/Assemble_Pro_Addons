...
def get_date(freq, date):
    ...
    if freq == 'monthly':
        ...
        return (rec_date - relativedelta(months=1))+ relativedelta(months=1)
    elif freq == 'quarterly':
        return (rec_date - relativedelta(months=3))+ relativedelta(months=3)
    elif freq == 'twice_year':
        return (rec_date -relativedelta(months=6)) + relativedelta(months=6)
    elif freq == 'annually':
        return (rec_date - relativedelta(years=1)) + relativedelta(years=1)
...
class legal_compliance(models.Model):
    ...
    notify_day = fields.Integer(string = "Reminder to be received before:" , default='0')
            ...
            if rec.due_date and rec.schedule_period:
                ...
                if date_notify.date() == date.today():
                    ...
                    if rec.performed_user_id.email :
                        ...
                        if rec.schedule_period == 'monthly' and mail:
                            # print "hhhhhhhhhhhhhhhhhhhh", rec.due_date
                            rec.write({'due_date': (date_compliance + relativedelta(months=1))})
                            # print "mata rani  KKKKKKKKKKKKKKKKKKKKKKKK" , rec.due_date
                            
                        if rec.schedule_period == 'quarterly' and mail:
                            rec.write({'due_date': (date_compliance + relativedelta(months=3))})
                            # print "lalita KKKKKKKKKKKKKKKKKKKKKKKKKKKKK" , rec.due_date
                            
                        if rec.schedule_period == 'twice_year' and mail:
                            rec.write({'due_date': (date_compliance + relativedelta(months=6))})
                            # print "lalita KKKKKKKKKKKKKKKKKKKKKKKKKKKKK" , rec.due_date
